...
import { getSortContent } from '@/apis/sort';
...
const Sort = () => {
	...
	const [sortLinks, setSortLinks] = useState<Link[]>([]);

	useEffect(() => {
		const fetchData = async () => {
			try {
				if (typeof docTitle === 'string') {
					const sortList = await getSortContent(docTitle);
					setSortLinks(sortList);
				}
			} catch (error) {
				alert('🦁문서를 찾을 수 없습니다🦁');
	...
	return (
		<Main>
			...
			<Viewer>
				...
				<ViewerBody>
					<ContentsHeader>
						<Title>{`분류: ${docTitle}`}</Title>
					</ContentsHeader>
					<ContentsBody>
					<Content>
						{sortLinks.map((link, index) => (
							<List key={index}>
								<Text onClick={() => isClickedLink({ title: link.title })}
								>&bull; {link.title}</Text>{' '}
							</List>
						))}
				</Content>
					</ContentsBody>
